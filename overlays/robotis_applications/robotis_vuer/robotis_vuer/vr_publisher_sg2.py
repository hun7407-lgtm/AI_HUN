...
class VRTrajectoryPublisher(Node):
    ...
    def __init__(self):
        ...
        self.declare_parameter('left_gripper_max_position', 1.1)
        self.declare_parameter('right_gripper_max_position', 1.1)
        ...
        # Command topics (gripper/lift/head) need RELIABLE delivery to match
        # the trajectory consumers; BEST_EFFORT here is silently dropped.
        self.vr_command_qos = QoSProfile(
    ...
    def publish_right_joystick(self, thumbstick_value):
        ...
        try:
            ...
            # Jog the lift once the stick is pushed past a moderate threshold.
            # (Was 0.95, which required pinning the stick fully to the edge and
            # made the lift feel unresponsive / "not working".)
            if not (raw_thumbstick_value < -0.5 or raw_thumbstick_value > 0.5):
                return
    ...
    async def on_controller_move(self, event, session):
        ...
        try:
            ...
            # TEMP DEBUG: report A/B button detection and available key names.
            _btn_dbg = (left_a, right_a, left_b, right_b)
            _prev_btn_dbg = getattr(self, '_btn_dbg_prev', None)
            if _btn_dbg != _prev_btn_dbg:
                _lkeys = (
                    sorted(self.left_controller_state.keys())
                    if isinstance(self.left_controller_state, dict) else []
                )
                _rkeys = (
                    sorted(self.right_controller_state.keys())
                    if isinstance(self.right_controller_state, dict) else []
                )
                self.get_logger().info(
                    f'[BTN_DEBUG] left_a={left_a} right_a={right_a} '
                    f'left_b={left_b} right_b={right_b} both_a={both_a_now} '
                    f'left_keys={_lkeys} right_keys={_rkeys}'
                )
            ...
            if not both_squeezes_now:
                self._publish_reactivate(False, reason='squeeze released')
            self.both_squeezes_prev = both_squeezes_now
