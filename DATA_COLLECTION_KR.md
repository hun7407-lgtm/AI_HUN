...
대시보드에서 SG2 태스크 6개를 선택할 수 있다. mobile 하나를 제외하면 전부 **고정(용접)
베이스**다:

| # | 대시보드 라벨 | 태스크 ID | 베이스 |
|---|---|---|---|
| 1 | Basket Pick & Place | `Cyclo-Real-Pick-Place-FFW-SG2-v0` | 고정 |
| 2 | L-Table Pick & Place (thin box) | `Cyclo-Real-Pick-Place-LTable-FFW-SG2-v0` | 고정 |
| 3 | **L-Table Pick & Place (mobile base)** **[추가]** | `Cyclo-Real-Pick-Place-LTable-Mobile-FFW-SG2-v0` | **자유(주행)** |
| 4 | Box Stack (thick box) | `Cyclo-Real-Box-Stack-FFW-SG2-v0` | 고정 |
| 5 | Single Box Far (rear table) | `Cyclo-Real-Single-Box-Far-FFW-SG2-v0` | 고정 |
| 6 | Single Box Far (thick box) | `Cyclo-Real-Single-Box-Far-Thick-FFW-SG2-v0` | 고정 |

**mobile(주행) 베이스는 L-Table Pick & Place 태스크에만 적용**되어 있다(3번). 각 태스크에는
증강 파이프라인(§7)이 쓰는 `…-Mimic-…` env와 `FFW_SH5`(핸드) 버전도 있다. 이 문서는 L-Table
태스크를 예시로 쓰지만, 녹화/증강/변환 흐름은 다른 고정 베이스 태스크에서도 동일하다.

| L-Table 변형 | 베이스 이동 | 차원 |
|---|---|---|
| `…-Pick-Place-LTable-FFW-SG2-v0` (고정) | scripted L-motion (키네마틱 텔레포트) | 19 |
| `…-Pick-Place-LTable-Mobile-FFW-SG2-v0` **[추가]** | 작업자가 `/cmd_vel`로 주행 (물리 스워브) | 22 |
...
연결 방식은 두 가지다. **USB를 권장**한다 — WiFi보다 지연 지터가 낮다(WiFi는 순간적으로
지터가 튀어 teleop 중 팔/베이스 떨림으로 나타남).
...
**WiFi:** Quest 브라우저에서 2단계 — `https://<PC-IP>:8012`에서 인증서 수락 후,
`https://vuer.ai/?ws=wss://<PC-IP>:8012`를 열어 Enter VR. WiFi 전체 절차는 베이스 레포 README에
정리되어 있다: [`EKAIWORKER`](https://github.com/Disniekie01/EKAIWORKER) → *VR Teleoperation —
How to Connect*.

일반적인 VR 원격조작 세팅(Vuer, 헤드셋, 퍼블리셔/컨트롤러)은 ROBOTIS 가이드를 따른다:
<https://docs.robotis.com/docs/systems/aiworker/quick_start_guide/operation_guide/vr_teleoperation/>.
...
1개, 19차원이었다. 추가 2가지로 실물 스키마에 맞춘다.
...
고정 베이스(베이스 레포) 형태에서는 베이스가 **scripted L-motion으로 로봇 root를 키네마틱
텔레포트**해서 이동한다. 여기서 두 가지 문제가 생긴다:
- 텔레포트가 물리 시뮬과 충돌한다 — root가 텔레포트되는 동안 들고 있는 박스는 여전히 물리로
  움직여서, 그리퍼 대비 박스가 **흔들리는(jitter)** 문제가 있다.
- 이 키네마틱 텔레포트 베이스 모션이 **데이터 생성(IsaacLab-Mimic datagen)에서 깨끗하게
  이어지지 않아서**, 모바일 베이스 궤적을 안정적으로 증강할 수 없었다.

텔레포트-물리 충돌을 피하려고, **물리로 주행하는** 베이스 USD(`FFW_SG2_MOBILE`)를 만들었다:
베이스가 root 텔레포트 없이 실제 스워브 바퀴를 물리로 굴려 이동하므로, 베이스와 들고 있는
박스가 물리적으로 일관되게 유지된다.
...
- **카메라 4개 렌더는 GPU 부하가 크다.** 검증 워크스테이션(**NVIDIA RTX PRO 5000 Blackwell
  Laptop, 24 GB**)에서는 일반 사용 시 프레임드랍이 나타나지 않으며, 관찰된 드랍은 일시적이었다.
  다른/저사양 워크스테이션에서는 카메라 4개 렌더가 GPU를 포화시켜 프레임드랍이 생길 수 있고,
  이는 teleop을 느리게 한다. 단, 프레임드랍은 조작감에만 영향을 주고 **녹화 데이터엔 영향
  없다**: 각 프레임은 여전히 해당 시뮬 시점의 올바른 이미지이며 시뮬이 실시간보다 느리게 돌
  뿐이다. (저사양에서 필요하면 recorder를 headless로 돌려 GUI 뷰포트 렌더를 제거 — 데이터엔
  영향 없음.)
